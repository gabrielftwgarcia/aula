...
public class PaisDAO extends TemplatePaisDAO{
    ...
    // Implementando métodos de TemplatePaisDAO
    ...
    public PaisDTO getPaisById(int id) {
        ...
        try (Connection conn = DriverManager.getConnection("jdbc:derby:memory:database")) {
            ...
            pstm.setInt(1, id);
            log.info("Selecionando um pais por id...");
    ...
    // ***********************************************
    public PaisDTO getPaisByNomeSigla(String nome, String sigla) {
        PaisDTO pais = new PaisDTO();

        try (Connection conn = DriverManager.getConnection("jdbc:derby:memory:database")) {

            String sql = "SELECT * FROM pais WHERE nome=? AND sigla=?";

            PreparedStatement pstm = conn.prepareStatement(sql);
            pstm.setString(1, nome);
            pstm.setString(2, sigla);
            log.info("Selecionando um pais por nome e sigla...");
            ResultSet rs = pstm.executeQuery();
